...
def copy_business_document_to_client_folder(enquiry_document_url, client_id, trade_name=None, business_name=None):
    ...
    try:
        ...
        return {
            'url': result['secure_url'],
            'public_id': result['public_id'],
            'format': result.get('format', 'unknown'),  # Format will be determined by file content
            'bytes': result['bytes'],
...
def create_client():
    try:
        ...
        client_data = {
            ...
            'assigned_staff': [user_email] if user_email else [],  # Auto-assign creator as assigned staff
            **extracted_data,
            **data  # Form data should override extracted data
        }
        
        print(f"📧 Auto-assigned creator as staff: {user_email}")
...
def update_client(client_id):
    try:
            ...
            try:
                ...
                try:
                    ...
                    if EMAIL_SERVICE_AVAILABLE and email_service:
                        ...
                        print("⚠️ Email service not available - skipping notification")
        ...
        # Determine update type for email notification
        update_type = "updated"
        if status is not None:
            if status.lower() == "interested":
                update_type = "marked as interested"
            elif status.lower() == "not_interested":
                update_type = "marked as not interested"
            elif status.lower() == "hold":
                update_type = "marked as hold"
            elif status.lower() == "pending":
                update_type = "marked as pending"
            elif status.lower() == "processing":
                update_type = "marked as processing"
            else:
                update_type = f"status changed to {status}"
        elif feedback is not None:
            update_type = "feedback updated"
    ...
    try:
        ...
        if WHATSAPP_SERVICE_AVAILABLE and client_whatsapp_service:
            try:
                ...
                # Comment-only updates are handled separately to avoid duplicates
                if not is_comment_only_update:
                    ...
                    print(f"Skipping multiple update messages for comment-only update")
            except Exception as e:
                print(f"Error sending WhatsApp notification: {str(e)}")
