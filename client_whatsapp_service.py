...
Thank you !"""
                ...
                # Send specific messages based on approval status
                if newly_approved_gateways:
...
Thank you!"""
                    ...
                    # General payment gateway update - don't send message for pending status
                    logger.info(f"No significant payment gateway status change for {legal_name}, not sending notification")
                    return {'success': True, 'message': 'No significant status change, notification skipped'}
...
Thank you!"""
    ...
    def send_loan_status_update_message(self, client_data: Dict[str, Any], loan_status: str) -> Dict[str, Any]:
        """
        Send WhatsApp message when loan status is updated
        
        Args:
            client_data (dict): Client information
            loan_status (str): New loan status
        ...
        try:
            ...
            # Create message with the requested template
            message = f"""Hi {legal_name} Sir/Madam,

Great to connect with you! Your loan status is {loan_status}.

If you have any questions or need assistance, please feel free to reply to this message."""
            ...
            # Log the activity
            if result['success']:
                logger.info(f"Sent loan status update message to {legal_name} at {mobile_number} - Status: {loan_status}")
            else:
                logger.error(f"Failed to send loan status update message to {mobile_number}: {result.get('error')}")
            
            return result
            
        except Exception as e:
            logger.error(f"Error in send_loan_status_update_message: {str(e)}")
            return {
                'success': False,
                'error': f'Error: {str(e)}'
            }
