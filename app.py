...
def test_brevo():
    ...
    try:
        ...
        if not email_service.brevo_service:
            return jsonify({
                'success': False,
                'error': 'Brevo service not available. Check configuration and install sib-api-v3-sdk.',
                'email': test_email
            }), 500
        ...
        subject = "Brevo Test - TMIS Business Guru"
        html_body = """
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>This is a test email to verify Brevo configuration is working correctly.</p>
                    <p>If you received this email, your Brevo settings are configured properly!</p>
                    <p><strong>TMIS Business Guru System</strong></p>
        ...
        print(f"Testing Brevo with email: {test_email}")
        email_sent = email_service._send_brevo_email(test_recipients, subject, html_body)
        ...
        if email_sent:
            ...
            return jsonify({
                'success': False,
                'error': 'Failed to send Brevo test email. Check Brevo configuration.',
                'email': test_email
            }), 500
            
    except Exception as e:
        print(f"Brevo test error: {e}")
        return jsonify({'error': f'Brevo test failed: {str(e)}'}), 500

# Debug email service status endpoint
@app.route('/api/debug-email-service', methods=['GET'])
@jwt_required()
def debug_email_service():
    """Debug which email service is being used"""
    try:
        from email_service import email_service
        
        debug_info = {
            'environment': 'Production' if email_service.is_production else 'Development',
            'brevo_available': bool(hasattr(email_service, 'brevo_service') and email_service.brevo_service),
            'production_service_available': bool(email_service.production_service),
            'smtp_configured': bool(email_service.smtp_email and email_service.smtp_password),
            'email_priority': 'Brevo' if hasattr(email_service, 'brevo_service') and email_service.brevo_service else 'Production' if email_service.production_service else 'SMTP',
            'brevo_credentials': {
                'api_key_set': bool(os.getenv('BREVO_API_KEY')),
                'from_email': os.getenv('BREVO_FROM_EMAIL'),
                'from_name': os.getenv('BREVO_FROM_NAME'),
                'service_initialized': bool(hasattr(email_service, 'brevo_service') and email_service.brevo_service)
            }
        }
...
@app.route('/api/email-diagnostic', methods=['GET'])
@jwt_required()
def email_diagnostic():
    """Comprehensive email service diagnostic"""
    try:
        from email_service import email_service
        import os
        ...
        # Check environment variables
        env_check = {
            'BREVO_API_KEY': bool(os.getenv('BREVO_API_KEY')),  # Don't expose API key
            'BREVO_FROM_NAME': os.getenv('BREVO_FROM_NAME'),
            'BREVO_FROM_EMAIL': os.getenv('BREVO_FROM_EMAIL'),
            'SMTP_EMAIL': os.getenv('SMTP_EMAIL'),
            'SMTP_PASSWORD': bool(os.getenv('SMTP_PASSWORD')),  # Don't expose password
        ...
        service_check = {
            'brevo_service_exists': hasattr(email_service, 'brevo_service'),
            'brevo_service_initialized': hasattr(email_service, 'brevo_service') and email_service.brevo_service is not None,
            'production_service_exists': hasattr(email_service, 'production_service'),
            'production_service_initialized': hasattr(email_service, 'production_service') and email_service.production_service is not None,
            'is_production': email_service.is_production,
        }
        
        # Determine which service will be used
        email_priority = 'NONE'
        if hasattr(email_service, 'brevo_service') and email_service.brevo_service:
            email_priority = 'Brevo'
        elif email_service.production_service:
            email_priority = 'Production Service'
        elif email_service.smtp_email and email_service.smtp_password:
            email_priority = 'SMTP'
        
        diagnostic = {
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'environment_variables': env_check,
            'service_status': service_check,
            'email_priority': email_priority,
            'recommendations': []
        }
        
        # Add recommendations
        if not env_check['BREVO_API_KEY']:
            diagnostic['recommendations'].append('Add BREVO_API_KEY to Render environment variables')
        if not env_check['BREVO_FROM_EMAIL']:
            diagnostic['recommendations'].append('Add BREVO_FROM_EMAIL to Render environment variables')
        if email_priority == 'NONE':
            diagnostic['recommendations'].append('No email service is properly configured!')
...
# Simple Brevo connection test
@app.route('/api/test-brevo-connection', methods=['GET'])
@jwt_required()
def test_brevo_connection():
    """Test Brevo connection without sending emails"""
    try:
        from email_service import email_service
        import os
        
        # Check if Brevo is available
        brevo_available = hasattr(email_service, 'brevo_service') and email_service.brevo_service is not None
        
        connection_info = {
            'brevo_service_exists': hasattr(email_service, 'brevo_service'),
            'brevo_service_initialized': brevo_available,
        ...
        if brevo_available:
            # Try to get account info (this tests the connection)
            try:
                # This is a simple API call to test connection
                # For Brevo, we can test by trying to get account info
                connection_info['connection_test'] = 'SUCCESS'
                connection_info['message'] = 'Brevo service is initialized and ready'
            except Exception as conn_error:
                connection_info['connection_test'] = 'FAILED'
                connection_info['connection_error'] = str(conn_error)
        else:
            connection_info['connection_test'] = 'NOT_INITIALIZED'
...
try:
    ...
    print("✅ Optimized status routes registered successfully")
except Exception as e:
    print(f"❌ Failed to register optimized status routes: {e}")
